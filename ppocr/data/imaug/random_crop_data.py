...
class EastRandomCropData(object):
    ...
    def __call__(self, data):
        ...
        w_scale = w_size / crop_w   # width scale factor
        h_scale = h_size / crop_h   # height scale factor
        ...
        if self.keep_ratio:
            ...
            final_w, final_h = w_size, h_size  # Use target size directly
        ...
        for poly, text, tag in zip(text_polys, texts, ignore_tags):
            # Shift polygon coordinates relative to crop origin
            poly = poly - np.array([crop_x, crop_y])
            
            if self.keep_ratio:
                # Scale uniformly when keeping ratio
                scaled_poly = (poly * scale).tolist()
            else:
            ...
            if not is_poly_outside_rect(scaled_poly, 0, 0, final_w, final_h):
